...
from pipeline.datasets.load import generate_patches, load_eval_data, generate_patches_RX
from pipeline.datasets.datasets import train_RX,test_data
...
from pipeline.loss.l1_loss import l1_loss_weight
from pipeline.loss.adversarial_loss import disc_loss, gen_loss
from pipeline.loss.log_likelihood import log_likelihood_pol
from pipeline.trainer.trainer_AAE_RX import TrainerAAE_RX
from pipeline.scheduler.ae import Scheduler_AE
from pipeline.storage.state import StateStorageFile

class Config_AAE_RX(ConfigMultiNN):
    def __init__(self,params):
        patches, patches_RX = generate_patches_RX(params['train_dir'],
                                                  params['train_RX_dir'],
                                                  pat_size=params['pat_size'],
                                                  stride=params['stride'],
                                                  bat_size=params['batch_size'],
                                                  nb_ch=params['channels'],
                                                  rx_tresh=params['tresh_RX'],
                                                  binarize=params['bin_RX'])
        
        process = transforms.Compose([normalization(params['norm'][0],params['norm'][1]),ToTensor()])
        process_RX = ToTensor()
        train_patches = train_RX(patches,patches_RX,process,process_RX)
        
        eval = load_eval_data(params['eval_dir'])
        process_eval = transforms.Compose([normalization(params['norm'][0],params['norm'][1]),ToTensor()])
        eval_dataset = test_data(eval,process_eval,nb_ch=params['channels'])
        ...
        self.rec_loss = l1_loss_weight(params['norm'][0],params['norm'][1],
