...
class TrainerAAE_RX:
    ...
    def train_step(self, input_data: torch.Tensor, RX_map: torch.Tensor):
        input_data = move_to_device(input_data, device=self.device)
        RX_map = move_to_device(RX_map, device=self.device)
        ...
        rec_loss = self.rec_loss(input_data, model_output,RX_map)
        rec_loss.backward()
    ...
    def predict_step(self ,input_data: torch.Tensor, label: torch.Tensor ,name: str ,epoch_id, step_id ,denorm):
        ...
        loss = torch.mean(torch.abs(result-torch.squeeze(input_data)))
    ...
    def run_train_epoch(self, epoch_id: int):
        ...
        for step_id, (input_data, RX_map) in enumerate(self.train_data_loader):
            rec_loss, disc_loss, gen_loss = self.train_step(input_data,RX_map)
